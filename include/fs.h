...
struct filetable {
  ...
  int fileptr; //block
  struct dirent *de;
  struct inode in; //sequence number
  int flag;
...
struct fsystem {
  ...
  int blocksz;
  int ninodes;
  int inodes_used;//inode-1
  int freemaskbytes;//freemask-size
  char *freemask;	//points to the entry of next free point
